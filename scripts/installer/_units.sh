...
write_systemd_units() {
  ...
  # canonical daemon. KillMode=control-group reaps the whole tree (no zombies).
  # Restart=always + a generous StartLimit keep it up without a tight crash loop.
...
ExecStartPre=-/usr/bin/pkill -f sync_daemon.py
