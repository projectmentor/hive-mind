...
# ── remember + dedup ────────────────────────────────────────────────────────
sect "remember + dedup"
"$HV" remember "Sam prefers parallel delegation" --tags workflow,preference >/dev/null
DUP="$("$HV" remember "Sam prefers parallel delegation" --tags workflow,preference)"
assert_contains "repeat content corroborates (not duplicated, not inflated)" "$DUP" "Corroborated"
"$HV" remember "Acme is a real estate team" --tags business,real-estate >/dev/null
ROWS="$(py <<'PY'
import os,sqlite3
c=sqlite3.connect(os.path.join(os.environ["HIVE_HOME"],"store.db"))
print(c.execute("SELECT count(*) FROM facts WHERE content LIKE 'Sam prefers%'").fetchone()[0])
PY
)"
